...
function theory_done_for_topic(array $theoryDoneMap, string $topicName): bool
{
    $topicVal = $theoryDoneMap[$topicName] ?? null;

    $topicSlug = function_exists('slugify_ua')
        ? slugify_ua($topicName)
        : mb_strtolower(trim($topicName));
                                ...
                                <div class="test-card__right">
                                    <?php if (!$locked): ?>
                                        <form method="post" action="/account/quiz.php">
                                            ...
                                            <input type="hidden" name="test_id" value="<?php echo (int)$tid; ?>">
                                            <button class="btn btn--primary" type="submit">Почати</button>
                                        </form>
                                    <?php elseif (!$theoryDone): ?>
                                        <a class="btn btn--primary"
                                           href="/account/theory.php?topic=<?php echo rawurlencode($topicName); ?>&go_test_id=<?php echo (int)$tid; ?>">
