...
/**
 * ✅ ДОДАНО: user_upsert — саме цього не вистачало.
 * Приймає повний user-масив і:
 * - якщо існує id → оновлює
 * - якщо не існує → створює з дефолтами
 */
...
function user_set_plan(string $userId, string $plan, int $days): bool {
  ...
  $store = users_load();
  if (!isset($store['users']) || !is_array($store['users'])) {
    $store = ['users' => is_array($store) ? $store : []];
  }
